...
function openPrintTiled() {
  ...
  if (hasSelection) {
    // Используем текущий фрагмент
    sourceCanvas = document.createElement('canvas');
    sourceCanvas.width = selectionImg.naturalWidth || 400;
    sourceCanvas.height = selectionImg.naturalHeight || 400;
    sourceCanvas.getContext('2d').drawImage(selectionImg, 0, 0);
  } else {
    // Используем всю страницу PDF
  ...
  tiledData.img = sourceCanvas.toDataURL('image/jpeg', 0.92);

  // Рассчитываем физический размер (если есть калибровка)
  const units = getUnitsPerMM();
  if (units) {
    tiledData.w_mm = sourceCanvas.width / units;
    tiledData.h_mm = sourceCanvas.height / units;
  } else {
    // Если нет калибровки, за базовый берем A0
    tiledData.w_mm = 841;
    tiledData.h_mm = 1189;
  }
...
async function createTiledPdfDoc() {
  ...
  try {
    ...
    vCanvas.width = target.w * scaleFactor;
    vCanvas.height = target.h * scaleFactor;
...
const initialLeft = rect.left; // This might need adjustment if right-aligned

// 2. Create Overlay element that mimics the button
const overlay = document.createElement('div');
overlay.id = 'measurementsOverlay';
overlay.className = 'measurements-overlay';

// 3. Set initial inline styles to match button EXACTLY
// We use fixed positioning to break out of layout flow
overlay.style.position = 'fixed';
overlay.style.top = initialTop + 'px';
overlay.style.left = initialLeft + 'px'; // We will animate left/width
overlay.style.width = initialWidth + 'px';
overlay.style.height = initialHeight + 'px';
overlay.style.borderRadius = '8px'; // Assuming button radius
overlay.style.transformOrigin = 'top right'; // Expand from right

// Keep content structure similar initially
const lines = getLines();
const validLines = lines.filter(l => l.points && l.points.length >= 2);
const count = validLines.length;

// Inner HTML structure with hidden list initially
overlay.innerHTML = `
    <div class="overlay-header">
      <div class="header-content">
        <span class="header-title">Всего: ${formatLength(getLinesTotalLength())}</span>
        <span class="header-subtitle">Замеров: ${count}</span>
      </div>
      <button class="close-overlay-btn">&times;</button>
    </div>
    
    <div class="overlay-list-wrapper">
      <div id="measurementsListContainer" class="overlay-list">
        <!-- Items -->
      </div>
       <div class="overlay-footer">
        <button id="exportMeasurementsBtn" class="btn-text-icon">
          💾 Сохранить список
        </button>
      </div>
    </div>
...
document.body.appendChild(overlay); // Body to be on top of everything

// 4. Inject Dynamic Styles (if not exists)
const styleId = 'measurements-morph-style';
if (!document.getElementById(styleId)) {
  ...
  style.innerHTML = `
      ...
      .measurements-overlay.expanded {
        /* Target dimensions will be set via JS or max-width classes */
        box-shadow: 0 15px 40px rgba(0,0,0,0.5);
      }
      
      .overlay-header {
        height: ${initialHeight}px; /* Match button height */
        display: flex;
        align-items: center;
        ...
        flex-shrink: 0;
        cursor: pointer; /* Click header to close? */
      }
      ...
      .header-subtitle { font-size: 11px; opacity: 0; transition: opacity 0.2s; height: 0; overflow:hidden;}
      ...
      .header-title { font-weight: 700; white-space: nowrap; font-size: 13px; text-overflow: ellipsis; overflow: hidden; }
      .header-subtitle { display: none; } /* Hide count to save space in narrow mode */
      
      .close-overlay-btn {
        background: none; border: none; color: #fff; font-size: 16px; 
        min-width: 20px; cursor: pointer; opacity: 0.7;
      }
      .close-overlay-btn:hover { opacity: 1; }
      ...
      .overlay-list-wrapper {
        ...
        display: flex;
        flex-direction: column;
        /* Start hidden, show after expand delay */
      }
      .measurements-overlay.expanded .overlay-list-wrapper {
        opacity: 1;
      ...
      .overlay-list::-webkit-scrollbar { width: 2px; } /* Thinner scrollbar */
      .overlay-list::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.2); border-radius: 2px; }
      ...
      .measure-item:hover { background: rgba(255,255,255,0.05); }
      .measure-item.active { background: rgba(124, 92, 255, 0.15); color: #fff; }
      .measure-item .idx { color: rgba(255,255,255,0.4); margin-right: 4px; font-size: 10px; }

      .overlay-footer { padding: 8px; border-top: 1px solid rgba(255,255,255,0.1); }
      .btn-text-icon {
        width: 100%; padding: 6px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1);
        border-radius: 4px; color: #fff; font-size: 12px; cursor: pointer; transition: all 0.2s;
        text-align: center;
      }
      .btn-text-icon:hover { background: rgba(124, 92, 255, 0.2); border-color: rgba(124, 92, 255, 0.4); }
    `;
...
lines.forEach((line) => {
  ...
  let len = 0;
  for (let i = 0; i < line.points.length - 1; i++) len += distance(line.points[i], line.points[i + 1]);
  const valStr = formatLength(len / units);

  const div = document.createElement('div');
  div.className = 'measure-item';
  if (line === selectedLine) div.classList.add('active');
  // Compact Item Layout
  div.innerHTML = `<span class="idx">#${validLineCount}</span><span style="font-weight:600">${valStr}</span>`;
  div.onclick = (e) => {
    e.stopPropagation();
    selectedLine = line;
    overlay.querySelectorAll('.measure-item').forEach(el => el.classList.remove('active'));
    div.classList.add('active');
    scheduleDraw();
  };
  container.appendChild(div);
});
if (validLineCount === 0) container.innerHTML = '<div style="text-align:center;padding:10px;color:#aaa;font-size:11px">Нет замеров</div>';

// Compact export button
const exportBtn = overlay.querySelector('#exportMeasurementsBtn');
exportBtn.onclick = downloadMeasurements;
exportBtn.innerHTML = '💾 Скачать'; // Shorten text
...
// 7. ANIMATE EXPANSION
// User requested: "button just expands down", NO WIDTH CHANGE. 
// We keep width exactly as the button width.
...
requestAnimationFrame(() => {
  ...
  overlay.style.height = 'auto'; // allow content to dictate, but max-height restricted by CSS
  // Instead of auto, we might need a specific height or max-height. 
  // Let's set a sufficient max-height in pixels to allow transition if we want smooth height.
  // Or just set height to '400px' (capped by max-height in CSS).
  // Actually, 'auto' doesn't transition well.
  // We can estimate height: header (rect.height) + list (items * 30) + footer (40).
  const estimatedHeight = Math.min(rect.height + (validLines * 35) + 50, 400);
  overlay.style.height = Math.max(estimatedHeight, rect.height) + 'px';
...
setTimeout(() => {
  const clickOutside = (e) => {
    if (!overlay.parentElement) return; // already removed
    if (!overlay.contains(e.target)) {
      closeMeasurementsList(overlay, counterBtn);
...
function closeMeasurementsList(overlay, counterBtn) {
  ...
  overlay.style.left = rect.left + 'px';
  overlay.style.top = rect.top + 'px';
  overlay.style.borderRadius = '8px'; // Back to button border radius

  // Fade out list content immediately
  const wrapper = overlay.querySelector('.overlay-list-wrapper');
  if (wrapper) wrapper.style.opacity = '0';
  ...
  setTimeout(() => {
    ...
    counterBtn.style.pointerEvents = 'all';
